...
最終更新: 2026-09-17 / 現行バージョン: v2.6.2
...
- **v2.5** (2026-08-09) — `journal.html` に 2026-08-09(約3か月)エントリ(`#d-260809`)を追加。両品種そろって記録。原本 `photo/260809マイヤー.JPG` / `photo/260809りのか.JPG`、Web 版 `photo/web/` 同名 `.jpg`(長辺 1400px / q85)。今回は各品種 1 枚(全体)+ キャプション `▲ 全体`。所見: マイヤー=7月のエカキムシ・摘花を経ても樹勢良好、新梢が支柱の頂点を追い越して夏芽を展開、下葉の黄化はほぼ解消。中段の葉のまだら・凹凸は**以前からある物理的な傷跡(擦れ・打ち傷)でユーザーが病害でないと判断済み**。すす病との混同を避けるため「傷跡はそのまま残す/**新しく**出た黒ずみ・ベタつきだけがすす病のサイン」と書き分けた(`index.html#susu` `#kaigara` `#aburamushi` へリンク)。璃の香=摘花後も落葉なく上部の葉が肉厚・濃緑に改善、樹形は細身のままで中間部はスカスカ、新たな蕾なし。**土際近くの細長い緑色は種苗登録ラベルであって芽ではない**(白ラベルと2枚)、台木芽は見当たらず枝分かれは全て接ぎ木部より上の正規の側枝、と明記。注意は「ラベルの紐を外す/今後 接ぎ木部より下から出た芽は掻き取る」の一般則に留める(`index.html#g_taigime` `#g_tsugikibu` へリンク)。**両品種とも、この2か月ほぼ毎日アゲハが飛来して新芽・若葉に産卵しており、毎日テデトール(手取り)で卵を除去、孵化した幼虫も都度除去している**という日々の管理作業を両セルに記載(`index.html#ageha` へリンク、発生は 10 月頃まで続く旨も)。両者共通で「8月は追肥なし・9月下旬〜10月のお礼肥まで待つ」(`index.html#feeding`)、猛暑期の朝夕2回の水やり・台風対策(`index.html#aug`)、支柱の結束ひもと品種ラベルの紐の食い込み確認を記載
- **v2.6** (2026-09-17) — `journal.html` に 2026-09-13(約4か月)エントリ(`#d-260913`)を追加。両品種そろって記録、各品種 1 枚(全体)+ キャプション `▲ 全体`。原本 `photo/260913マイヤー.JPG` / `photo/260913りのか.JPG`(EXIF 撮影日時 2026-09-13 11:32、Orientation=6 の横向き。`ImageOps.exif_transpose` で正立させてから縮小)、Web 版 `photo/web/` 同名 `.jpg`(長辺 1400px / q85)。所見: マイヤー=主幹先端が支柱の頂点をはっきり追い越し、上半分は濃緑の成葉が密、枝先に秋芽を複数展開、下葉の黄化はほぼ解消。中〜上段の数枚に**葉縁の欠け・小穴・かじり跡**があり 1 枚は**黄白色に色が抜けて内巻きになった葉**(巻き葉はエカキムシ被害の典型だが白い蛇行状の筋は写真では未確認 → 現物確認待ち)。下段の黄色いまだらは v2.5 で現物確認済みの**物理的な傷跡**と同一・不拡大と明記。璃の香=細身・中間部スカスカは変わらないが**先端に黄緑の秋芽がまとまって伸長**、上部の葉は厚み・照りを維持、落葉なし。今回の新事実として**土際から 10〜20cm の幹下部から新しい枝が出て着葉**(葉が 1 枚葉でカラタチ台木の三出葉と異なるため台木芽ではなさそう、ただし位置が低いので**接ぎ木部より上か要現物確認**という書き方に留めた。`index.html#g_taigi` `#g_taigime` `#g_tsugikibu` へリンク)。**土表面に緑色の苔・藻**がうっすら → 水のやり過ぎ/日照不足のサインとして「表土が乾いてから水やり+よく日に当てる」を注意に追加。ラベル 2 枚と緑の結束ひもは幹に接したままで食い込み前の対応を継続宿題として記載。両品種共通で、9 月は `index.html#sep`(秋の成長期・秋芽は虫の標的・台風が最大の脅威)を参照し、**9 月下旬のお礼肥**(`index.html#feeding`、株元から 3〜5cm 離して四方に置く)、アゲハは `index.html#oct` 頃まで継続、エカキムシの葉裏チェックを記載。v2.5 と違い「毎日のアゲハ手取り」は写真から確認できないため記述せず、かじり跡から幼虫チェックを促す書き方にした
- **v2.6.1** (2026-09-17) — 同日訂正。璃の香 2026-09-13 エントリの「幹の下部から新しい枝が出て葉を付け始めた(台木芽か要確認)」は**誤り**。鉢の土に挿さっている茶色い棒は<b>ユーザーが手入れ用に挿した割り箸</b>(拡大すると先端に割り箸特有の割れ目と角ばった木肌が見える)で、芽でも枝でもない。また幹の低い位置の葉は 8/09 の写真にも写っており新規のものではない。該当箇所を「割り箸である/低い位置の葉は以前から/台木芽は今回も見当たらない」に書き換え、注意も「今後 接ぎ木部より下から芽が出たら掻き取る(カラタチ台木は三出葉で見分けやすい)+割り箸やラベルは写真では芽と紛らわしいので現物で確かめる」の一般則に修正。**教訓: 鉢まわりの人工物(v2.5 の種苗登録ラベル、v2.6 の割り箸)を芽・枝と誤認しやすい。前回写真と見比べて「今回新しく現れたもの」かどうかを必ず確認し、断定を避ける**
- **v2.6.2** (2026-09-17) — ユーザーの実観察を反映。マイヤー 2026-09-13 エントリに「葉の欠け・穴の**一部はアゲハの幼虫によるもの**だが、8月に続いて**毎日 卵と幼虫を手で取り除いており(テデトール)被害を最小限に食い止めている**」旨を、v2.5 と同じ書式(写真所見のあとに `<br>` で日々の管理作業の段落を置く)で追記。注意側は「かじり跡はアゲハ幼虫の可能性があるのでチェック」という推測の書き方をやめ、「アゲハの発生は 10 月頃まで続くので毎日の卵・幼虫チェックを続ける(卵は1mm前後の黄白色の球)」という継続行動の書き方に変更。璃の香側は今回ユーザー言及がないため変更なし
