...
- **v2.3** (2026-07-11) — 同日追記。マイヤーの 2026-07-11 エントリに 3 枚目の写真として **摘み取ったエカキムシ被害葉** を追加(原本 `photo/260711マイヤーエカキムシ.JPG`、Web 版同名 `.jpg`)。白い蛇行状の食害跡の中に黄緑色の幼虫が数匹写っている。メモに「被害のひどい葉 2 枚を摘み取った」旨を追記
- **v2.4** (2026-07-18) — `journal.html` に **璃の香の番外編** 2026-07-18 エントリ(`#d-260718`)を追加。7/11 に確認した初の蕾が開花 → マイヤー(7/04)と同じく 1 年目方針で摘花、という **璃の香として初の開花&摘花** の記録。原本 `photo/260718りのか花.JPG` / `photo/260718りのか摘花.JPG`、Web 版 `photo/web/` 同名 `.jpg`(長辺 1400px / q85)。v2.2 以降の書式どおり 1 セルに写真 2 枚(開花・摘花)を縦積みしキャプション付き。マイヤー側は「記録なし」プレースホルダ。日付セルは `(番外編・璃の香 初開花&摘花)` と明記。摘花メモは `index.html#basic`(1年目方針)、エカキムシ警戒は `index.html#hamoguri` へリンク。目次に日付行を追加
